...
'''
    ...
    for opus_track in tracks:
        ...
        # check for unterminated notes (OisГ­n) -- 5.2
...
def _clean_up_warnings():  # 5.4
    ...
    if _previous_times > 1:
        print('  previous message repeated '+str(_previous_times)+' times',
         file=sys.stderr)
...
def _some_text_event(which_kind=0x01, text='some_text'):
    return b'\xFF'+bytes((which_kind,))+_ber_compressed_int(len(text))+bytes(text, encoding='ascii')
    ...
    while(len(trackdata)):
        ...
        if (first_byte < 0xF0):  # It's a MIDI event
            ...
            if (command      == 0x00):
                ...
                # try:  # 4.9 decode fails if it's not valid utf8
                text_str = trackdata[0:length].decode('ascii','ignore')
                # except UnicodeDecodeError:
                #     text_str = trackdata[0:length].decode('latin1')
                # Defined text events
                if (command == 0x01):
            ...
            # Now the sticky events -------------------------------------
                 ...
                 E = ['sequencer_specific',time,trackdata[0:length].decode()]
            else:
                 E = ['raw_meta_event', time, command,
                       trackdata[0:length].decode()]
    ...
    for event_r in (events):
        ...
        if (   # MIDI events -- eligible for running status
            ...
            if event == 'raw_meta_event':
                ...
                #event_data = _some_text_event(0, _int2twobytes(E[0]))
